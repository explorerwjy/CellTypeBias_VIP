...
def RandomGenes(ExpMat, WeightDF, outdir, GeneProb, n_sims=10000):
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    ExpMat = pd.read_csv(ExpMat, index_col=0)
    valid_genes = ExpMat.index.values
    # Which default gene set to use
    WeightDF = pd.read_csv(WeightDF, header=None)
    ...
    if GeneProb != None:
        ...
        for i in range(n_sims):
            Genes = np.random.choice(valid_genes, size=len(Gene_Weights))
            tmp_dict = dict(zip(Genes, Gene_Weights))
            Dict2Fil(tmp_dict, "{}/cont.gw.{}.csv".format(outdir, i))


def SubsampleSibling_HumanCT(ExpMat, WeightDF, outdir, GeneProb, n_sims=10000):
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    SibWeightDF = pd.read_csv("/home/jw3514/Work/ASD_Circuits/dat/Unionize_bias/sibling_weights_LGD_Dmis.csv", header=None)
    ExpMat = pd.read_csv(ExpMat, index_col=0)
    valid_genes = ExpMat.index.values
    SibGenes = SibWeightDF[0].values
    SibGenes = [g for g in SibGenes if g in valid_genes]
...
def AdjustClusterMean(BiasDF, HumanCT_Z2_HCT_ColMean):
    for i, row in BiasDF.iterrows():
        BiasDF.loc[i,"EFFECT_Adj"] = row["EFFECT"] - HumanCT_Z2_HCT_ColMean[i]
    return BiasDF
...
def CtrlBiasCal_HumanCT(GW_Dir, SpecMat, outDir, n_processes=20): 
    print("HumanCT Bias Calculation")
    if not os.path.exists(outDir):
        os.makedirs(outDir)
    Ctrl_GW_Fils = []
...
def GetOptions():
    ...
    parser.add_argument('--exp_cutoff', type=float, default=0.01, help="Expression cutoff")
...
def main():
    ...
    if mode == 'gw':
        ...
        RandomGenes(SpecMat, WeightDF, outdir, GeneProb, n_sims)
        #SubsampleSibling_HumanCT(SpecMat, WeightDF, outdir, GeneProb, n_sims)
    if mode == 'bias':
        GW_Dir = args.GW_Dir
        SpecMat = args.SpecMat
        outDir = args.outdir
